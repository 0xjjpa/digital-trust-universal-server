...
const {
  error, CONSENT_PATH, INTERACTION_PATH, CLIENT_ID, CLIENT_ID_ALT,
  DEFAULT_REQUEST_OBJECT, REQUEST_WITH_CLAIMS,
  getInteractionIdFromInteractionUri, PAYLOAD_AUTH_ALT
} = require('./fixtures')
const assert = require('assert')
const { notStrictEqual, deepStrictEqual, strictEqual } = assert
    ...
    describe(text, function () {
      it('should return a code on success', async function () {
        ...
        deepStrictEqual(this.appender.output, [
          `[consent] [DEBUG] Consent Success, Client Id ${CLIENT_ID}, Interaction Id: ${interactionId}, ` +
          'Accepted: {"id_token":{"claims":{"given_name":0},"assertions":{}},' +
          '"userinfo":{"claims":{},"assertions":{}},"approved_scopes":["openid"]}',
          ...
          `[authorize] [DEBUG] Authorization Success with Client ID: ${CLIENT_ID}`
        ])
      })
      ...
      it('should fail when the user does not match with the one store in the current interaction', async function () {
        ...
        deepStrictEqual(this.appender.output, [
          '[consent] [DEBUG] User does not match. ' +
          `Client Id: ${CLIENT_ID}, Interaction Id: ${interactionId}`
        ])
      })
